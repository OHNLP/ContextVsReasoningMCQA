...
def apply_permute_answer(row, model=None, model_path=None):
    ...
    for k, v in options.items():
        ...
        prompt = (f"Select the appropriate option from the provided question.\n Respond in the format A or B.\n\n"
                  f"The question:\n{question}\n Is the answer {v} correct? Other possible answers "
                  f"are {', '.join(other_opts)} \n\n"
                  f"Options:\nA: Yes\nB: No")
        messages = [
            {
                "role": "user",
...
def call_experimental_setting_2_1(model_name, dataset_name, working_dir):
    tqdm.pandas()
    ...
    df.to_csv(f'data/outputs/{dataset_name}/{working_dir}/{model_name}/step_6_self_consistency_permuted_new.csv')
