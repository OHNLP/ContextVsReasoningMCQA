...
def shuffle_options(row):
    ...
    output_row = row.copy()
    output_row['shuffled_context_options'] = output_option_dict
    output_row['shuffled_context_answer_idx'] = answer_idx
    return output_row


def apply_qa(row, model=None, model_path=None) -> str:
    question: str = row['question']
    options: dict = row['shuffled_context_options']
    prompt: str = question + "\n Options: \n"
    for k, v in options.items():
        prompt = prompt + k + ': ' + v + '\n'
    messages = [
...
def call_experimental_setting_1_1(model_name, dataset_name, working_dir):
    tqdm.pandas()
    ...
    df = df.progress_apply(shuffle_options, axis=1)
    for i in range(0, 5):
        df[f'shuffled_context_model_answer_{i}'] = df.progress_apply(lambda r: apply_qa(r, model, model_name), axis=1)
    df['shuffled_context_model_answer'] = ''
    df['shuffled_context_answer_correct'] = ''
    output_dir = f'data/outputs/{dataset_name}/{working_dir}/{model_name}'
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
    df.to_csv(f'data/outputs/{dataset_name}/{working_dir}/{model_name}/step_9_shuffled_context.csv')
